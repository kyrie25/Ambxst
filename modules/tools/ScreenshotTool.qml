...
PanelWindow {
    ...
    function open() {
        if (Screenshot.captureModes.includes(GlobalStates.screenshotCaptureMode)) {
            screenshotPopup.currentMode = GlobalStates.screenshotCaptureMode;
            if (modeGrid) {
                var idx = screenshotPopup.modes.findIndex(mode => mode.name === screenshotPopup.currentMode);
                modeGrid.currentIndex = idx !== -1 ? idx : 0;
            }
        } else {
